...
Assert-True (($list.Output | Where-Object { $_ -match '^\[STEP\] tools:cloud-postgres-quality-contract .*lock=services\\reflex-cloud\\uv\.lock .*test_reflex_cloud_postgres_quality_release_smoke\.py -q' }).Count -eq 1) "Cloud PostgreSQL quality smoke contracts must use the frozen Cloud environment."
Assert-True (($list.Output | Where-Object { $_ -match '^\[STEP\] tools:history-upgrade-contract .*lock=packages\\reflex-runtime\\uv\.lock .*--extra dev --extra builtins pytest tools\\tests\\test_history_upgrade_smoke\.py -q' }).Count -eq 1) "History upgrade contract tests must use the frozen Runtime environment with built-in plugins."
Assert-True (($list.Output | Where-Object { $_ -match '^\[STEP\] tools:cross-version-history-upgrade-contract .*lock=packages\\reflex-runtime\\uv\.lock .*--extra dev --extra builtins pytest tools\\tests\\test_cross_version_history_upgrade_smoke\.py -q' }).Count -eq 1) "Cross-version history upgrade contract tests must use the frozen Runtime environment with built-in plugins."
