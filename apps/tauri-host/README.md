...
Rust Host 提供 `diagnostic_bundle_export` 与 `diagnostic_bundle_cancel`：

- 命令只授予主窗口，前端领域桥不接收路径或上传参数；
- 保存位置由系统原生对话框选择，诊断包不会自动上传；
- Host 只读取固定轮转文件，导出前重新校验字段、删除请求 ID 并执行二次敏感扫描；
- 导出使用有界 ZIP、同目录临时文件和原子替换，取消、失败或异常退出不会覆盖旧目标；
- 具体按钮由前端集成阶段按当前界面设计接入，不能绕过 Host 的扫描与权限边界。
