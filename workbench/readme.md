...
以上证据绑定的是 2026-08-29 当前工作树，不绑定某个提交，不能替代仍未完成的正式发布门禁。当前版本的 100 次短浸泡证据见 `workbench\runtime-soak-smoke-20260829.json`；每次提交必须先完成与改动范围匹配的本地隔离验证；后端 workflow 已支持 push、pull request 和手动触发，默认 push/PR 执行常规代码门禁和 Windows 生命周期工具契约，短浸泡 smoke、依赖实审和 SBOM 通过手动 `run_heavy` 入口执行；正式 Windows 生命周期和 72 小时浸泡仍需独立发布门禁实跑，分支保护是否实际要求该 workflow 仍需仓库设置确认。
