...
项目已完成架构契约冻结与 `reflex-core` Stage 1，当前进入 Runtime Sidecar 与 Mock 链路准备阶段。
...
- Core 自动测试：21 passed；
- Core 导入不引入 PyQt、PySide、sqlite3、torch 或 sentence-transformers。
...
进入 Stage 2：Runtime Sidecar 与 Mock 链路。
...
6. 覆盖多请求隔离、取消、非法消息、协议版本和进程退出测试；
7. 不接真实 MiniMax，不创建 Tauri 视觉页面。
