...
| P2-011 | PostgreSQL 多实例质量发布并发验证 | 待验证 | P2-009 | 烟测工具契约已通过；需在独立 PostgreSQL 测试库完成真实发布/回滚并发证据 |
...
- 本机当前可用虚拟内存约 1.2 GB，未启动额外 PostgreSQL 容器；P2-011 保持“待验证”，不能把契约测试代替真实 PostgreSQL 证据。
