...
- 从已修复反馈创建、发布和回滚质量指导，并按用户授权归因版本效果；
...
REFLEX_CLOUD_PRIVACY_POLICY_VERSION
...
| 匿名质量曝光 | 默认关闭 | 对比基线和质量发布版本的反馈结果 | 随安装身份删除，不包含输入或输出正文 |
...
6. 在管理后台完成一条反馈的分类、复现、计划和修复状态流转。
7. 使用已修复反馈创建质量发布草稿，检查场景指导 JSON、敏感信息拒绝和来源校验。
8. 发布草稿，确认新请求响应包含质量版本头、客户端只显示安全元数据，且指导进入 Cloud 优化链路。
9. 开启匿名质量分析后提交反馈，确认版本效果进入 `by_quality_release`；未开启的安装不得进入该维度。
10. 回滚当前质量发布，确认恢复上一版或回到无质量指导的基线状态。
11. 用测试安装执行删除，确认旧令牌失效、质量曝光删除、新安装重新默认为关闭授权。
12. 做一次 Postgres 备份和恢复演练，记录恢复耗时和数据完整性结果。
13. 在独立测试数据库执行 PostgreSQL 多实例质量发布烟测，确认同一版本并发发布/回滚只产生一个安全终态。
...
质量发布可通过管理后台回滚，回滚恢复上一条被替代的发布；如果没有上一版，服务
返回基线状态。回滚不会删除发布历史或原始数据库记录，便于审计和效果复盘。

## 7. 发布结论

alpha.6 已具备单实例 Beta 的预算护栏、公网代理配置、备份恢复工具和容器验收证据；
云服务镜像使用固定 uv 版本和 `uv.lock` 构建，不在发布时重新选择依赖版本。
alpha.8 已统一桌面端、Core、Runtime 和云服务的版本元数据；新镜像仍需在 P4-002
可复现构建门禁中生成和复验。
alpha.8 开发分支已具备反馈来源关联、质量发布/回滚、优化指导注入、授权曝光归因和
客户端透明度链路；这些能力通过本地自动化和管理后台运行态验证，但尚未替代正式
域名、真实 PostgreSQL 多实例并发、外部告警和受控用户试用证据。
