...
Stage D 已完成历史工作视图、结果保存与评分、流式翻译和安全 Markdown 预览。增强能力均通过 Runtime 插件协议接入，Tauri Host 只负责权限同步、事件消费和用户交互，`reflex-core` 未增加宿主、存储、网络或重模型依赖。

本轮未读取或使用真实 Provider 密钥。真实 MiniMax 在线请求仍需用户在应用设置中保存密钥后执行。可选的 `plugins\semantic-detector` 仍为未装载的 L1 占位包；当前产品使用已覆盖 42 场景的 Core L0 规则识别，并在识别失败时回退通用场景。
...
- L1 本地语义模型插件尚未实现；该能力不是当前 builtins，也不影响 L0 场景识别和失败回退。

## 回退

- 可在设置中分别关闭 Translator 与 Markdown Preview，Core 优化主链保持可用。
- Translator 或 Markdown 插件不可用时，宿主保留当前结果，只显示固定错误并允许重试或关闭。
